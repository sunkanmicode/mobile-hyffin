...
export const CustomButton = ({
  ...
  const getBgColor = () => {
    ...
    if (primary) return " bg-teal-700 text-bold ";
  ...
  return (
    <TouchableOpacity
      className={`px-2 h-12 my-2  border border-gray-300 rounded-lg items-center justify-center  mt-1 py-3 ${getBgColor()}`}
      disabled={disabled}
      onPress={onPress}
      style={style}
    >
      <View className={` flex-row `}>
        ...
        {title && (
          <>
            <Text
              className={`${disabledTextBtn} font-[Inter] font-bold  text-sm  leading-[15px] ${getBgColor()}`}
            >
